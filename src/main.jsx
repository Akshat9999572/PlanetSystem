...
function PlanetModel({ planet, internal }) {
  ...
  if(internal) return <InternalPlanet planet={planet} texture={texture} size={size}/>
  return <group ref={mesh} rotation={[.1,-.55,0]}><mesh castShadow receiveShadow><sphereGeometry args={[size,112,112]}/><meshStandardMaterial map={texture} roughness={gaseous?.5:.82} metalness={planet.type==='ice'?.17:.02} bumpMap={texture} bumpScale={gaseous?.025:.07}/></mesh>{(gaseous||planet.id==='पृथ्वी'||planet.type==='ice')&&<CloudShell texture={texture} size={size} planet={planet}/>}<Atmosphere size={size} color={glow}/>{planet.type==='saturn'&&<Rings color="#e5d3a3"/>}<OrbitingMoons planet={planet} size={size}/><pointLight position={[2.5,1.8,2]} intensity={.32} color={planet.colors[2]}/></group>
}
...
function PlanetStage({ planet, internal }) {
  const controls=useRef(); const wrap=useRef()
  ...
  return <div className="stage-shell" ref={wrap}><div className="stage-caption"><Sparkles size={14}/> जीवंत 3D मॉडल</div><Canvas camera={{position:[0,0,6.3],fov:38}} shadows dpr={[1,2]}><color attach="background" args={['#f7f3ea']}/><ambientLight intensity={1.35}/><directionalLight position={[4,4,4]} intensity={2.4} castShadow/><pointLight position={[-4,-2,2]} intensity={.7} color="#ffcf83"/><Suspense fallback={null}><PlanetModel planet={planet} internal={internal}/><ContactShadows position={[0,-2.25,0]} opacity={.34} scale={6.4} blur={2.6} far={3.5}/><Stars radius={24} depth={4} count={220} factor={1.5} saturation={0} fade speed={.1}/></Suspense><CameraControls ref={controls}/></Canvas><div className="stage-controls"><button onClick={()=>controls.current?.rotate()} title="घुमाएं"><RotateCw/></button><button onClick={()=>controls.current?.zoom()} title="ज़ूम"><ZoomIn/></button><button onClick={()=>controls.current?.pan()} title="खिसकाएं"><Move/></button></div><button className="snapshot" onClick={snapshot}><Camera size={15}/> स्नैपशॉट</button></div>
}
